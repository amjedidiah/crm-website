...
async function createFile(file, fileInfo, verbose = false) {
  const { process, directory, location } = fileInfo;
  try {
    mkdirp(directory);
...
function updateWatchOptionsIgnoredPaths(
  { watchOptions: { ignored } },
  { outputDirectory, outputName }
) {
  const newPath = path.join("**", outputDirectory, outputName);

  if (Array.isArray(ignored)) {
    ignored.push(escapeStringRegexp(newPath));
  } else {
    ignored = new RegExp(ignored.source + "|" + escapeStringRegexp(newPath));
  }
}
/**
 * createApolloClient
